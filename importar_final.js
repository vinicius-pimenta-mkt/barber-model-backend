...
async function executar() {
    console.log('🚀 Iniciando busca pelo banco de dados...');
    const caminhosPossiveis = [
        path.join(__dirname, 'database', 'barbearia.db'),
        path.join(__dirname, 'barbearia.db'),
        path.join(process.cwd(), 'database', 'barbearia.db'),
        '/app/database/barbearia.db'
    ];
    let dbPath = null;
    ...
    if (!dbPath) {
        console.error('❌ Erro: Não encontrei o arquivo barbearia.db. Certifique-se de rodar o script na pasta do backend.');
        return;
    }
    console.log(`✅ Banco encontrado em: ${dbPath}`);
    try {
        const db = await open({ filename: dbPath, driver: sqlite3.Database });
        await db.exec(`CREATE TABLE IF NOT EXISTS clientes (id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT NOT NULL, telefone TEXT, email TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
        let inseridos = 0;
        let pulados = 0;
        for (const c of clientesParaImportar) {
            const existe = await db.get('SELECT id FROM clientes WHERE telefone = ?', [c.telefone]);
            if (!existe) {
                await db.run('INSERT INTO clientes (nome, telefone, email) VALUES (?, ?, ?)', [c.nome, c.telefone, c.email]);
                inseridos++;
            } else { pulados++; }
        }
        console.log(`\n✨ Sucesso! Novos: ${inseridos}, Já existiam: ${pulados}`);
        await db.close();
        console.log('\n👉 Reinicie o backend na Easypanel e atualize o navegador.');
