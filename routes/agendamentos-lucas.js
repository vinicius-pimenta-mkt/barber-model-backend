...
const padronizarPreco = (precoRaw) => {
  ...
  if (typeof precoRaw === 'number') return precoRaw < 1000 ? Math.round(precoRaw * 100) : Math.round(precoRaw);
...
router.get('/', verifyToken, async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ error: 'Erro ao buscar agendamentos do Lucas' }); }
});
...
router.get('/disponibilidade', async (req, res) => {
  try {
    ...
    res.json({ livres });
  } catch (error) { res.status(500).json({ error: 'Erro ao buscar disponibilidade' }); }
});

router.post('/', async (req, res) => {
  try {
    const { cliente_nome, cliente_telefone, servico, data, hora, status, preco, forma_pagamento, observacoes } = req.body;
    if (!cliente_nome || !servico || !data || !hora) return res.status(400).json({ error: 'Campos obrigatórios faltando' });
    ...
    const horarioOcupado = await get("SELECT id FROM agendamentos_lucas WHERE data = ? AND hora = ? AND status != 'Cancelado'", [data, hora]);
    if (horarioOcupado && status !== 'Bloqueado') return res.status(400).json({ error: 'Horário indisponível. O Lucas já tem um cliente neste horário.' });
...
router.put('/:id', verifyToken, async (req, res) => {
  try {
    ...
    const horarioOcupado = await get("SELECT id FROM agendamentos_lucas WHERE data = ? AND hora = ? AND status != 'Cancelado' AND id != ?", [data, hora, id]);
    if (horarioOcupado && status !== 'Bloqueado') return res.status(400).json({ error: 'Horário indisponível.' });
    ...
    if (status === 'Confirmado') {
      try {
        ...
        if (telefoneLimpo) await query('UPDATE assinantes SET ultima_visita = ? WHERE telefone = ? OR nome = ?', [dataVisita, telefoneLimpo, cliente_nome]);
        else await query('UPDATE assinantes SET ultima_visita = ? WHERE nome = ?', [dataVisita, cliente_nome]);
      } catch (e) {}
    }
    res.json({ message: 'Agendamento do Lucas atualizado' });
  } catch (error) { res.status(500).json({ error: 'Erro ao atualizar agendamento' }); }
});
