...
router.get('/', verifyToken, async (req, res) => {
  try {
    ...
    let queryText = 'SELECT * FROM agendamentos_jhonatas';
    const params = [];
    const conditions = [];
    ...
    res.status(500).json({ error: 'Erro ao buscar agendamentos do Jhonatas' });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    const horarioOcupado = await get(
      "SELECT id FROM agendamentos_jhonatas WHERE data = ? AND hora = ? AND status != 'Cancelado'",
      [data, horaFormatada]
    );

    if (horarioOcupado && status !== 'Bloqueado') {
      return res.status(400).json({ error: 'Horário indisponível. O Jhonatas já tem um cliente neste horário.' });
    }

    const safeClienteId = cliente_id || null;

    const result = await query(
      'INSERT INTO agendamentos_jhonatas (cliente_id, cliente_nome, cliente_telefone, servico, data, hora, status, preco, forma_pagamento, observacoes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    res.status(201).json({ id: result.lastID, message: 'Agendamento criado para o Jhonatas' });
  } catch (error) {
    res.status(500).json({ error: 'Erro ao criar agendamento do Jhonatas' });
  }
});
...
router.put('/:id', verifyToken, async (req, res) => {
  try {
    ...
    const horarioOcupado = await get(
      "SELECT id FROM agendamentos_jhonatas WHERE data = ? AND hora = ? AND status != 'Cancelado' AND id != ?",
      [data, hora, id]
    );
    ...
    await query(
      'UPDATE agendamentos_jhonatas SET cliente_nome=?, cliente_telefone=?, servico=?, data=?, hora=?, status=?, preco=?, forma_pagamento=?, observacoes=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
    ...
    res.json({ message: 'Agendamento do Jhonatas atualizado' });
  } catch (error) {
    res.status(500).json({ error: 'Erro ao atualizar agendamento do Jhonatas' });
  }
});

router.delete('/:id', verifyToken, async (req, res) => {
  try {
    await query('DELETE FROM agendamentos_jhonatas WHERE id = ?', [req.params.id]);
    res.json({ message: 'Agendamento do Jhonatas deletado' });
  } catch (error) {
    res.status(500).json({ error: 'Erro ao deletar agendamento do Jhonatas' });
  }
});
