...
// Endpoint resumo - dados para a página de relatórios do Jhonatas
router.get('/resumo', verifyToken, async (req, res) => {
  try {
    ...
    console.error('Erro ao buscar resumo de relatórios do Jhonatas:', error);
    res.status(500).json({ error: 'Erro interno do servidor' });
  }
});

// Dashboard - dados gerais do Jhonatas
router.get('/dashboard', verifyToken, async (req, res) => {
  try {
    const hoje = new Date().toISOString().split('T')[0];

    // Buscar dados do dashboard
    const agendamentosHoje = await all('SELECT COUNT(*) as total FROM agendamentos_jhonatas WHERE data = ?', [hoje]);
    const receitaHoje = await all('SELECT SUM(preco) as total FROM agendamentos_jhonatas WHERE data = ? AND status = ?', [hoje, 'Confirmado']);
    const proximosAgendamentos = await all('SELECT * FROM agendamentos_jhonatas WHERE data >= ? ORDER BY data, hora LIMIT 5', [hoje]);
    const servicosRealizados = await all('SELECT COUNT(*) as total FROM agendamentos_jhonatas WHERE data = ? AND status = ?', [hoje, 'Confirmado']);
    ...
    console.error('Erro ao buscar dados do dashboard do Jhonatas:', error);
    res.status(500).json({ error: 'Erro interno do servidor' });
  }
});
